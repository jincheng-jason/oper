...
<div class="container-fluid" id="main-container">
	...
	<ul class="breadcrumb">
		<li><i class="icon-home"></i> <a>用户管理</a><span class="divider"><i class="icon-angle-right"></i></span></li>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										<td>${var.phone_num}</td>
										<td>${var.password}</td>
										<td><div style="width:50px;column-width: auto;" title=${var.avata}><a href="${var.avata}" target="_blank"><img src="${var.avata}"/></a></div></td>
										<td>${var.nick_name}</td>
										<td>${var.weibo}</td>
										<td>${var.weixin}</td>
										<td>${var.append}</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					var url = "<%=basePath%>/client/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>/client/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
