    ...
    <div id="page-content" class="clearfix">
        ...
        <div class="row-fluid">
            ...
            <div class="row-fluid">
                ...
                <!-- 检索  -->
                <form action="event/lasting.do" method="post" name="Form" id="Form">
                    <table>
                        <tr>
                            <td>
                                ...
                                <c:if test="${QX.cha == 1 }">
                                    <c:forEach items="${varList}" var="var" varStatus="vs">
                                        <tr>
                                            ...
                                            <td><div style="width:100px;column-width: auto;max-height:100px;overflow:hidden;over-flow:hidden" title=${var.large_img}><a href="${var.large_img}" target="_blank"><img src="${var.large_img}"/></a></div></td>
                                            <td><div style="width:100px;column-width: auto;max-height:100px;overflow:hidden;over-flow:hidden" title=${var.small_img}><a href="${var.small_img}" target="_blank"><img src="${var.small_img}"/></a></div></td>
                                                <%--<td style="width:60px;" class="center"><label><input type="checkbox" class="ace-switch ace-switch-3" id="isChief" <c:if test="${var.status == true }">checked="checked"</c:if> onclick="chief(${var.ID},this.checked)" /><span class="lbl"></span></label></td>--%>
                                            <td>${var.begin_at}</td>
                                            <td>${var.end_at}</td>
                            ...
                            <tr>
                                <td style="vertical-align:top;">
                                    <c:if test="${QX.add == 1 }">
                                        ...
                                        <a class="btn btn-small btn-danger" onclick="makeAll('确定要下线选中的活动吗?');">下线</a>
                                    </c:if>
...
<script type="text/javascript">
    ...
    function del(Id){
        bootbox.confirm("确定要删除吗?", function(result) {
            if(result) {
                var url = "<%=basePath%>/event/delete.do?id="+Id+"&tm="+new Date().getTime();
    ...
    function edit(Id){
        ...
        diag.Drag=true;
        diag.Title ="编辑";
        diag.URL = '<%=basePath%>/event/goEdit.do?id='+Id;
        diag.Width = 650;
        diag.Height = 555;
        diag.CancelEvent = function(){ //关闭事件
            if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
        ...
        bootbox.confirm(msg, function(result) {
            if(result) {
                ...
                if(str==''){
                    ...
                    }else if(msg == '确定要下线选中的活动吗?'){
                        $.ajax({
                            type: "POST",
                            url: '<%=basePath%>/event/onlineAll.do?status=0',
                            data: {DATA_IDS:str},
                            dataType:'json',
                            //beforeSend: validateData,
                            cache: false,
